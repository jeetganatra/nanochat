...
USER_MSG_TEMPLATES = [
    ...
    "{word} में {letter} कितनी बार आता है",
    "{word} शब्द में {letter} की गिनती बताओ",
    "{word} में {letter} गिनो",
    # Gujarati
    "{word} માં કેટલા {letter} છે",
    "{word} માં {letter} કેટલી વાર આવે છે",
    "{word} શબ્દમાં {letter} ગણો",
    "{word} માં {letter} ની સંખ્યા કેટલી છે",
    # Marathi
    "{word} मध्ये किती {letter} आहेत",
    "{word} मध्ये {letter} किती वेळा येतो",
    "{word} शब्दात {letter} मोजा",
    "{word} मध्ये {letter} ची संख्या सांगा",
    # Telugu
    "{word} లో ఎన్ని {letter} ఉన్నాయి",
    "{word} లో {letter} ఎన్ని సార్లు వస్తుంది",
    "{word} అనే పదంలో {letter} లెక్కించండి",
    "{word} లో {letter} సంఖ్య ఎంత",
    # Tamil
    "{word} இல் எத்தனை {letter} உள்ளன",
    "{word} இல் {letter} எத்தனை முறை வருகிறது",
    "{word} என்ற வார்த்தையில் {letter} எண்ணுங்கள்",
    "{word} இல் {letter} எண்ணிக்கை என்ன",
]
...
class SpellingBee(Task):
    ...
    def get_example(self, index):
        ...
        if rng.random() < 0.5: # fair assumption that 50% of people don't even use question marks
            user_msg += "?"
...
""" 
        ...
        # TODO: this can be made more sophistacated where we
        # simulate model making mistakes and self correcting
