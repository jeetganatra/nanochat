...
from tasks.smoltalk import SmolTask
...
train_tasks = [
    SmolTask(split="train"), # 460K rows of general conversations
...
val_dataset = TaskMixture([
    SmolTask(split="test"), # 24K rows in test set
...
def sft_data_generator_bos_bestfit(split, buffer_size=100):
    ...
    bost_token = tokenizer.get_bos_token_id()
        ...
        for _ in range(args.device_batch_size):
            ...
            while len(row) < row_capacity:
                ...
                if best_idx >= 0:
                    ...
                    # No conversation fits, pad the remaining instead of croppping
                    # We dont want to discard any tokens
                    content_len = len (row)
                    row.extend([bost_token]*remaining)
...
while True:
    ...
    lrm = get_lr_multiplier(progress=progress)
